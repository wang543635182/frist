...
if [ -n "${REPLICATION_MASTER}" ]; then
    ...
    if [ ! -f $VOLUME_HOME/replication_set.1 ]; then
...
log-bin=mysql-bin">>  /etc/mysql/conf.d/mysql.cnf
        touch $VOLUME_HOME/replication_set.1
    else
...
if [ -n "${STARTUP_SQL}" ]; then
    if [ ! -f $VOLUME_HOME/sql_imported ]; then
        echo "=> Initializing DB with ${STARTUP_SQL}"
        ImportSql
        touch $VOLUME_HOME/sql_imported
    fi
fi
...
#    if [ ! -f $VOLUME_HOME/replication_set.2 ]; then
...
#        echo "=> Done!"
#        touch $VOLUME_HOME/replication_set.2
#    else
#        echo "=> MySQL replication master already configured, skip"
#    fi
#fi
